...
const search = info => {
	// const url = "https://c.y.qq.com/soso/fcgi-bin/client_search_cp?w=" + "Smoke Filled Room" // change this if you want to hardcode search string
	const url = "https://c.y.qq.com/soso/fcgi-bin/client_search_cp?w=" + encodeURIComponent(info.keyword) // change this if you want to hardcode search string
  console.log(`[INFO] QQMusic search query: ${url}`) // Added
...
const single = (id, format) => {
  ...
  console.log(`[INFO] QQMusic single query: ${url}`) // Added
    ...
    midurlinfo[0].purl ? console.log(`[INFO] QQMusic single query returned something`) : console.log(`[INFO] QQMusic single query returned nothing`)
		// const vkey =
		// 	jsonBody.req_0.data.midurlinfo[0].vkey ||
		// 	(jsonBody.req_0.data.testfile2g.match(/vkey=(\w+)/) || [])[1]
		// return concatenate(vkey)
		return midurlinfo[0].purl ? sip[0].replace('http://','https://') + midurlinfo[0].purl : Promise.reject()
	})
}
...
const track = id => {
	...
	.catch(() => {
    console.log("[INFO] QQMusic track::catch\n")
